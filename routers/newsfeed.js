...
const getEnhancedxPosts = async (queryParameters) => {
  ...
  let baseQuery = `
        SELECT 
            ...
            u.firstname, u.lastname, u.email, u.universityid, un.university, c.interest
        FROM 
            posts p
        LEFT JOIN 
  ...
  baseQuery +=
    " GROUP BY p.postid, u.firstname, u.lastname, u.universityid, un.university, c.interest";
...
const getComments = async (postid) => {
  try {
    const query =
      "SELECT a.*,b.email FROM comments a LEFT JOIN users b ON a.comment_userid = b.userid WHERE postid = ? ";
